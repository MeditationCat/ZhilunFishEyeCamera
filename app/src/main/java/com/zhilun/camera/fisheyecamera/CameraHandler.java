...
import com.zhilun.camera.encoder.MediaAudioEncoder;
import com.zhilun.camera.encoder.MediaEncoder;
import com.zhilun.camera.encoder.MediaMuxerWrapper;
import com.zhilun.camera.encoder.MediaSurfaceEncoder;
import com.zhilun.camera.encoder.MediaVideoEncoder;
...
public class CameraHandler extends Handler {
	...
	public static final CameraHandler createHandler(final CameraActivity parent, final CameraViewInterface cameraView) {
    ...
    public String getSupportedSize() {
        final CameraThread thread = mWeakThread.get();
        return thread.getSupportedSize();
    }
...
/*		public void release() {
	...
	private static final class CameraThread extends Thread {
		...
		private CameraThread(final CameraActivity parent, final CameraViewInterface cameraView) {
			super("CameraThread");
        ...
        //get camera config
        public String getSupportedSize() {
            if (DEBUG) Log.i(TAG, "CameraThread:->" + ((mUVCCamera != null) ? mUVCCamera.getSupportedSize() : "mUVCCamera = null"));
            return (mUVCCamera != null) ? mUVCCamera.getSupportedSize() : "";
        }
...
//					mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_YUV);
		...
		public void handleCaptureStill() {
			...
			if (mUVCCamera == null || parent == null) return;
			...
			try {
				...
				try {
					try {
                        ...
                        Toast.makeText(mWeakParent.get().getApplicationContext(), R.string.still_image_captured, Toast.LENGTH_SHORT).show();
		...
		public void handleStartRecording() {
			...
			try {
				...
				if (true) {	// only one encoder can access to audio device.
					// for audio capturing
					new MediaAudioEncoder(mMuxer, mMediaEncoderListener);
				}
				mMuxer.prepare();
				mMuxer.startRecording();
                Toast.makeText(mWeakParent.get().getApplicationContext(), R.string.recording_started, Toast.LENGTH_SHORT).show();
            } catch (final IOException e) {
		...
		public void handleStopRecording() {
			...
			if (mMuxer != null) {
                ...
                Toast.makeText(mWeakParent.get().getApplicationContext(), R.string.recording_stopped, Toast.LENGTH_SHORT).show();
                mMuxer = null;
		...
		public void handleUpdateMedia(final String path) {
			...
			if (parent != null && parent.getApplicationContext() != null) {
				...
				Log.w(TAG, "CameraActivity already destroyed");
